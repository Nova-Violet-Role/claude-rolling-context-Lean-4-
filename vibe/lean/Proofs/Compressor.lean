...
/-- **[INFRASTRUCTURE] D1 is MASKED BY D4 on well-formed traffic — proved, not
measured.** Measured, not claimed: this theorem **survived all 66 mutations**,
and that is expected rather than embarrassing — it asserts an EQUALITY between
two walks that any mutation to `badCut` perturbs on *both* sides at once. It
cannot fail by breaking the model, so it guards nothing; it EXPLAINS something.
Do not cite it as a guarantee. Deleting D1 changes nothing here, because D4
rejects the boundary first. This is the whole explanation for why a corpus built from realistic
